...
send_request() {
    ...
    # Get start time with microsecond precision if possible
    local start_time
    if date +%s.%N >/dev/null 2>&1; then
        # Use nanosecond precision if available
        start_time=$(date +%s.%N)
    else
        # Fallback to regular seconds
        start_time=$(date +%s)
    fi
    ...
    # Get end time with microsecond precision if possible
    local end_time
    if date +%s.%N >/dev/null 2>&1; then
        # Use nanosecond precision if available
        end_time=$(date +%s.%N)
        ...
        # Fallback to regular seconds
        end_time=$(date +%s)
        # Calculate integer milliseconds
        local response_time=$((1000 * (end_time - start_time)))
    fi
